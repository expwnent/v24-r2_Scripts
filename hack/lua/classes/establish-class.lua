...
function establishclass(unit,verbose)
...
-- Check if the persistent variabls are present for the unit
 ...
 if not persistTable.GlobalTable.roses.UnitTable[key]['Spells'] then
  persistTable.GlobalTable.roses.UnitTable[key]['Spells'] = {}
  for i,x in pairs(persistTable.GlobalTable.roses.SpellTable._children) do
   persistTable.GlobalTable.roses.UnitTable[key]['Spells'][x] = '0'
  end
 end
