...
local delayRun = require 'persist-delay'
  ...
  for j,y in ipairs(targets) do
   ...
   for k,z in ipairs(args.script) do
	...
	elseif z == '!CENTER' then 
	 script[k] = unit2.id
    elseif split(z,'_')[1] == '!VALUE' then
	 equation = split(args.value,',')[tonumber(split(z,'_')[2])]
	 equation = equation:gsub('%#selected',tostring(#selected))
	 accepted,denied = 0,0
	 ...
	 equation = equation:gsub('%#allowed',tostring(allowed))
	 equation = equation:gsub('%#rejected',tostring(rejected))
	 script[k] = getValue(equation,unitSelf,y)
	else 
	 script[k] = z
	end
   end
   if delay == 0 then 
    ...
    delayRun(delay,table.unpack(script))
   end
  end
 end
end
