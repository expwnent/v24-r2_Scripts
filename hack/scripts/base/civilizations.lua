...
local establishCivilization = require('civilizations.establish-civ')
...
--  print('Checking if civ has leveled up')
  ...
  elseif method == 'COUNTER' then
   ...
   if tonumber(chance) >= rnum then dfhack.run_script('civilizations/level-up '..tostring(civ.id)) end
  end
  checkmethod(civ)
 end
end
