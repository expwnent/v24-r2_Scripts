...
function upgradeitem(args,v,unit,dur,subtype)
 local sitems = {}
 if ((args.equipped and args.unit) and not args.item) then
...
-- Increase items number by one
  for _,x in ipairs(sitems) do
   x:setQuality(sid+1)
   if dur > 0 then delay(dur,'item/quality-change -item '..tostring(x.id)..' -downgrade') end
  end
 elseif args.downgrade then
-- Decrease items number by one
  for _,x in ipairs(sitems) do
   x:setQuality(sid-1)
   if dur > 0 then delay(dur,'item/quality-change -item '..tostring(x.id)..' -upgrade') end
  end
 else
-- Change item to new quality
  for _,x in ipairs(sitems) do
   ...
   if dur > 0 then delay(dur,'item/quality-change -item '..tostring(x.id)..' -quality '..tostring(sid)) end
  end
 end
end
...
if args.help then -- Help declaration
 print([[item/subtype-change.lua
  ...
  examples:
   item/quality-change -unit \\UNIT_ID -weapon ALL -equipped -quality 6 -dur 1000
   item/quality-change -weapon ITEM_WEAPON_GIANTS -all -downgrade
 ]])
 return
end
